...
// this function will print a character into a given position in the screen and
...
// we do not specify a position so the funciton will begin to print on the
// cursor position
...
/// @return the offset
int get_offset(int col, int row) { return (row * MAX_COL + col) * 2; }

/// @brief this function will return the cursor position
/// @return the offset is the number of memory case between the cursor and the
/// address 0xb8000
int get_cursor() {
  int offset;
  outb(REG_SCREEN_CTRL,
       0x0E); // the index 0x0e (14) is the the cursor high position
  offset = inb(REG_SCREEN_DATA) << 8; // in order to get the high byte value
  outb(REG_SCREEN_CTRL,
       0x0F); // the index 0x0f (15) is the the cursor low positon
